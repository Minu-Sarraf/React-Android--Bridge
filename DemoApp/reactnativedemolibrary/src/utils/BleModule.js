...
export default class BleModule {
  ...
  removeAllListeners = str => {
    return bleManagerEmitter.removeAllListeners(str);
  };
  ...
  start() {
    return new Promise((resolve, reject) => {
      BleManager.start({
        showAlert: false,
        restoreIdentifierKey: 'BleInBackground',
        queueIdentifierKey: 'BleQueueInBackground',
      })
        .then(() => {
          this.checkState();
          resolve();
   ...
   */
  fullUUID(uuid) {
    if (uuid.length === 4 && Platform.OS != 'ios') {
      return '0000' + uuid.toUpperCase() + '-0000-1000-8000-00805F9B34FB';
    }
    if (uuid.length === 8 && Platform.OS != 'ios') {
   ...
   * */
  disconnect(id) {
    BleManager.disconnect(id ? id : this.peripheralId)
      .then(() => {
        this.isConnecting = false; //蓝牙是否连接
  ...
  startNotification(id) {
    return new Promise((resolve, reject) => {
      // console.log( this.peripheralId, this.nofityServiceUUID[index], this.nofityCharacteristicUUID[index] );
      console.log('nofityCharacteristicUUID', this.nofityCharacteristicUUID);
      const peripheral = id || this.peripheralId;
      BleManager.startNotification(peripheral, 'FFB0', 'FFB2')
        .then(() => {
  ...
  stopNotification(index = 0) {
    return new Promise((resolve, reject) => {
      BleManager.stopNotification(this.peripheralId, 'FFB0', 'FFB2')
        .then(() => {
  ...
  write(data, index = 0) {
    ...
    return new Promise((resolve, reject) => {
      BleManager.write(
        this.peripheralId,
        this.writeWithResponseServiceUUID[index].toString(),
        this.writeWithResponseCharacteristicUUID[index].toString(),
        data,
      )
        .then(() => {
  ...
  writeWithoutResponse(data, index = 0) {
    return new Promise((resolve, reject) => {
      BleManager.writeWithoutResponse(
        this.peripheralId,
        this.writeWithoutResponseServiceUUID[index].toString(),
        this.writeWithoutResponseCharacteristicUUID[index].toString(),
        data,
      )
        .then(() => {
  ...
  read(index = 0) {
    return new Promise((resolve, reject) => {
      BleManager.read(
        this.peripheralId,
        this.readServiceUUID[index].toString(),
        this.readCharacteristicUUID[index].toString(),
      )
        .then(data => {
  ...
  getMacAddressFromIOS(data) {
    // let macAddressInAdvertising = data.advertising.kCBAdvDataManufacturerData.bytes;
    let mac = '';
    let macAddressInAdvertising = data.advertising.manufacturerData.bytes;
    ...
    if (
      ...
      mac = this.swapEndianWithColon(macAddressInAdvertising);
    }
    return mac;
  }
   ...
   * */
  swapEndianWithColon(bytes) {
    let format = '';
    for (let i = 6; i > 0; i--) {
      let byteStr = bytes[bytes.length - i].toString(16);
      byteStr.length == 1 && (byteStr = '0' + byteStr);
      format += byteStr;
      if (i != 1) {
        format += ':';
      }
